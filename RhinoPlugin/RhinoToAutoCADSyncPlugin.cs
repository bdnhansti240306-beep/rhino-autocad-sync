...
namespace RhinoExportPlugin
{
    ...
    public class ExportToAutoCADCommand : Command
    {
        ...
        protected override Result RunCommand(RhinoDoc doc, RunMode mode)
        {
            try
            {
                // Get target AutoCAD file from user
                var targetFile = GetTargetAutoCADFile(doc);
                if (string.IsNullOrEmpty(targetFile))
                    return Result.Cancel;

                // Create base sync folder if not exists
                ...
                // Convert objects to export data
                ...
                // Save to target-specific folder
        ...
        private string GetTargetAutoCADFile(RhinoDoc doc)
        {
            ...
            if (!string.IsNullOrEmpty(lastTarget) && File.Exists(lastTarget))
            {
                ...
                {
                    if (getOption.OptionIndex() == 1) // UseLast
                    {
                        return lastTarget;
                    }
                }
        ...
        private List<RhinoObject> GetObjectsToExport(RhinoDoc doc)
        {
            ...
            // Ask user if they want to export all or selected
            var getOption = new GetOption();
            ...
            if (getOption.OptionIndex() == 1) // All
            {
        ...
        private ExportObject ConvertRhinoObject(RhinoObject rhinoObj)
        {
            try
            {
                ...
                {
                    // Determine if it's a closed polysurface, open polysurface, or single surface
                    if (brep.IsSolid)
                    {
                ...
                {
                    // For other geometry types, try to get bounding box
        ...
        private object SerializeClosedPolysurface(Brep brep)
        {
            ...
            // Extract all faces with their surface data
        ...
        {
            // For single-face Brep (trimmed surface)
    ...
    // Additional command for quick sync to last target
    public class ExportToAutoCADLastCommand : Command
    {
        ...
        protected override Result RunCommand(RhinoDoc doc, RunMode mode)
        {
            try
            {
                ...
                // Try to get last target without user interaction
